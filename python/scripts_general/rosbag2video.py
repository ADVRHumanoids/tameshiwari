...
def videoconverter():
    ...
    for files in range(0,len(opt_files)):
        ...
        for topic, msg, t in bag.read_messages(topics='camera1/image'):
        #        print topic, 'at', str(t)#,'msg=', str(msg)
            # print msg.encoding
            try:
                if msg.format.find("jpeg")!=-1 :
                    if msg.format.find("8")!=-1 and (msg.format.find("rgb")!=-1 or msg.format.find("bgra")!=-1):
                ...
                try:
                        ...
                        elif msg.encoding.find("bgra8")!=-1 :
                            # print "found the bgra8 format"
                            pix_fmt = "bgr24"
                            #np_arr = np.fromstring(msg.data, np.uint8)
                            if opt_display_images:
